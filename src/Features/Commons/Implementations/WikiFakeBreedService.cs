...
public class WikiFakeBreedService : IWikiFakeBreedService
{
    public Task<List<Breed>> GetBySpeciesTypeAsync(SpeciesType speciesType)
    {
        return Task.Run(() =>
        {
            ...
            Dog.Add(new Breed { Id = "7", Name = "Greater Swiss Mountain" });
    ...
    public Task<Breed> GetByIdAsync(int key)
    {
        return Task.Run(() =>
        {
            return GetAll().Result.Where(x => x.Id.Equals(key.ToString())).FirstOrDefault(); ;
        });
    }

    public Task<List<Breed>> GetAll()
    {
        return Task.Run(()=>
        {
            var cat = GetBySpeciesTypeAsync(SpeciesType.Cat);
            var dog = GetBySpeciesTypeAsync(SpeciesType.Dog);

            var breeds = new List<Breed>();
            breeds.AddRange(cat.Result);
            breeds.AddRange(dog.Result);
